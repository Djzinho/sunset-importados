# Fontes de mídia — Sunset Importados

Este arquivo registra a origem das fotografias adicionadas ao site. Fotografias editoriais são usadas para direção visual e apresentação de categorias e **não representam estoque em tempo real nem registros da loja física da Sunset**.

## Fotografias editoriais em uso

- `site/assets/media/iphone-16-pro-v3.jpg`
  - Fonte: Unsplash
  - Origem: `photo-1727013884184-b313982327f3`
  - Resolução publicada: 1800 × 1200 px
  - Uso: editorial de iPhone e página de iPhones.

- `site/assets/media/iphone-dark-v3.jpg`
  - Fonte: Unsplash
  - Origem: `photo-1697636979792-fb057f6cbe8d`
  - Resolução publicada: 1800 × 1200 px
  - Uso: hero escuro, iPhone e página de dispositivos.

- `site/assets/media/apple-watch-v3.jpg`
  - Fonte: Unsplash
  - Origem: `photo-1680731096036-b5b606ea6172`
  - Resolução publicada: 1600 × 2000 px
  - Uso: Apple Watch e ecossistema Apple.

- `site/assets/media/macbook-airpods-v1.jpg`
  - Fonte: Unsplash
  - Origem: `photo-1724251280780-2ea2838916e6`
  - Uso: MacBook/AirPods na galeria editorial.

- `site/assets/media/airpods-macbook-v1.jpg`
  - Fonte: Unsplash
  - Origem: `photo-1514474959185-1472d4c4e0d4`
  - Uso: AirPods/MacBook na galeria editorial.

## Imagem de contexto de localização

- `site/assets/media/porto-uniao-centro-v1.jpg`
  - Fonte: Wikimedia Commons
  - Arquivo: `Centro, Porto União - SC, Brazil - panoramio.jpg`
  - Autor/atribuição informada na página de origem: `Roberto de vasconcel…`
  - Data da fotografia: 12/06/2014
  - Resolução original: 4000 × 3000 px
  - Licença: Creative Commons Attribution-ShareAlike 3.0 Unported (CC BY-SA 3.0)
  - Uso: fundo desfocado de contexto territorial na seção de localização.
  - Observação: **esta imagem não é apresentada como fachada da Sunset Importados**.

## Identidade original

- `site/assets/sunset-logo.webp`
  - Origem: arquivo da própria Sunset Importados fornecido para o projeto.
  - Uso: identidade em tamanho reduzido no header e footer.

## Política para próximas imagens

1. Priorizar fotos e vídeos oficiais da Sunset quando estiverem disponíveis em qualidade suficiente.
2. Manter fotografias editoriais de produto apenas como apoio visual e identificá-las como ilustrativas quando necessário.
3. Não utilizar imagens de concorrentes.
4. Não apresentar mockups, renders ou fotografias de banco como se fossem registros da loja física, equipe ou estoque real da Sunset.
5. Não identificar uma fotografia como fachada da Sunset sem confirmação visual suficiente.
6. Toda mídia de terceiros usada comercialmente deve ter origem e condições de reutilização registradas.
7. Imagens novas devem ser otimizadas para web e armazenadas localmente no repositório sempre que possível.

Referências de licença: Unsplash License e CC BY-SA 3.0 conforme a página de origem de cada arquivo.

## Licensed 3D product mockups
- `site/assets/media/iphone-mockup-ls-v1.jpg` — LS Graphics, Free iPhone 15 Pro Mockup. Source: https://www.ls.graphics/assets/free-iphone-15-pro-mockup — listed by the publisher as free for personal and commercial projects.
- `site/assets/media/macbook-mockup-ls-v1.jpg` — LS Graphics, MacBook Air 2022 Mockup. Source: https://www.ls.graphics/assets/macbook-air-2022-mockup — listed by the publisher as free for personal and commercial projects.
- These mockups are third-party design assets and are not presented as official Apple marketing artwork or as proof of Sunset inventory.

## Transparent product renders
- `site/assets/media/iphone-transparent-v1.png` — PurePNG iPhone X render/cutout, transparent PNG. Source: https://purepng.com/photo/28223/electronics-iphone-x — page states free for commercial use, no attribution required.
- `site/assets/media/macbook-transparent-v1.png` — PurePNG MacBook render/cutout, transparent PNG. Source: https://purepng.com/photo/24735/electronics-macbook — page states free for commercial use, no attribution required.
- `site/assets/media/apple-watch-transparent-v1.png` — PurePNG Apple Watch render/cutout, transparent PNG. Source: https://purepng.com/photo/29572/objects-apple-watch — page states free for commercial use, no attribution required.
- These are isolated third-party product renders used as visual mockups; they do not represent current Sunset inventory.

## Home — mockups fornecidos pelo usuário
- `site/assets/media/iphone-17-pro-max-user-v1.webp` — mockup de iPhone 17 Pro Max fornecido diretamente pelo usuário nesta conversa; otimizado para WebP preservando transparência.
- `site/assets/media/apple-ecosystem-user-v1.webp` — composição com iPad, Apple Watch e AirPods fornecida diretamente pelo usuário nesta conversa; otimizada para WebP preservando transparência.
- Uso: trilho horizontal da Home. As imagens funcionam como links visuais para `/iphones/` e `/dispositivos/` e não representam estoque em tempo real.
